...
const itHTML = unitTest.bind(null, '');
const xitHTML = unitTest.bind(null, 'skip');
const fitHTML = unitTest.bind(null, 'focus');

function unitTest(mode, name, { dom, expected }) {
  (mode === 'focus' ? test.only : test)(name, async({ page }) => {
    if (mode === 'skip')
      test.skip();
...
test('should add click listener', async ({ page }) => {
  await page.evaluate(() => {
